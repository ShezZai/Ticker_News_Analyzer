...
**Sentiment** (`sentiment/graph.py`): a single historical-precedent verdict node — one structured-output Gemini call weighs the cosine-nearest prior articles/insights (the `precedents`) and emits the buy/sell/hold `Verdict` directly (no analyst fan-out, no separate synthesis judge). The model is configurable via `SENTIMENT_VERDICT_MODEL` (default `gemini-2.5-flash-lite`). Stored in `article_sentiment` (the `model` column records the configured verdict model). Prompt: `analysts.render_verdict` / `SENTIMENT_VERDICT_PROMPT`. Optionally a cheap **`summarize` node** runs first (`SENTIMENT_SUMMARY_MODEL`, default off): it distills the raw, often long/multi-language/boilerplate-heavy article body into a decision-relevant brief that REPLACES the body fed to the verdict (raw kept under `content_full` for provenance). Prompt: `analysts.render_summary` / `ARTICLE_SUMMARY_PROMPT` (Langfuse `summarize-article`). Per-run A/B: `eval pipeline --summary-model`.
...
- `SENTIMENT_PRECEDENT_SOURCE` — retrieval mode for the historical-precedent verdict: `article` (default; top-5 similar articles) | `insights` (per-insight-box ANN) | `distilled-first` / `distilled-second` (distilled boxes tagged by first/second label; `-second` drops `DROP`-labelled). Plus `SENTIMENT_PRECEDENT_INSIGHTS_THRESHOLD` (0.7) / `..._LIMIT` (40). Per-run override: `eval pipeline --precedent-source`. See `docs/precedent-source-options.md`.
- `SENTIMENT_VERDICT_MODEL` — Gemini model for the verdict node (default `gemini-2.5-flash-lite`; set `gemini-2.5-flash` for the stronger model). The whole panel is now this one call.
- `SENTIMENT_SUMMARY_MODEL` — Gemini model for the optional pre-verdict article-summary pass (default unset ⇒ disabled; the verdict sees the raw body). Set e.g. `gemini-2.5-flash-lite` to distill the body into a noise-free brief before the verdict. Per-run override: `eval pipeline --summary-model lite|flash|<id>`. Adds one cheap LLM call per article; traced as the `summarize-article` generation.
...
- **Experiments run items in parallel — keep the task `async`.** langfuse 4.7.1's runner (`_run_task`) only `await`s a task if it returns a coroutine, so a **sync** `def` task body blocks the event loop and runs items one-at-a-time no matter what `max_concurrency` is set to (it only ever gates async evaluators). Both evals are async: `eval classify`'s task awaits its single LLM call directly; `eval pipeline`'s task is `async def` but its stage chain is blocking, so it offloads to a worker thread with `await asyncio.to_thread(...)` — `to_thread` copies the OTEL context and `propagate_attributes` is set inside the thread, so each item keeps its own trace. Tune with `--concurrency` (classify default 16; pipeline default 4, kept modest because each item fans out ~7 Gemini calls against the shared DB — the Gemini rate limiter is the real ceiling). Any new experiment task: make it `async`, give each item its own psycopg connection, and build the `CallbackHandler` fresh per item (none are thread-safe to share).
...
- **Langfuse naming is critical**: every Langfuse entity — traces, spans, generations, scores, datasets, experiments, prompts — must have a descriptive, informative name. Generic names (`span`, `generation`, `run`, `eval`) make the Langfuse UI and evals unreadable. Use the stable observation names listed in the Observability section above for pipeline stages; for new entities, name them after what they actually do (e.g. `classify-binary`, `sentiment-synthesis`, `precedent-retrieval`).

The knowledge graph in `graphify-out/` is stale (pre-refactor `scripts/` layout) — do not trust it until regenerated.

Use Context7 for best practices with LangFuse and LangGraph. For Langfuse specifically, query these Context7 library IDs: `/langfuse/langfuse-docs` (full product docs — tracing/observability, prompt management, datasets, experiments, scores/evals) and `/langfuse/langfuse-python` (Python SDK). Prefer these over training data for SDK signatures and config, since the SDK moves fast (e.g. `set_trace_io` is deprecated; current API centers on `get_client()` + `start_as_current_observation`).
